    ...
    <?php if (isset($message_profile)) { ?>
        ...
        window.addEventListener('DOMContentLoaded', (event) => {
            ...
            function updateFirstMessageId() {
                ...
                loadMoreMessagesButtonDisplay();
            }

            function loadMoreMessagesButtonDisplay() {                                                 // if there are no more messages to display, remove "load more messages" button
                $.ajax({
                    type: 'POST',
                    url: 'action/load_more_messages_button.php',
                    data: {
                        conversation_id: "<?php if (isset($conversation)) echo $conversation->id; ?>",
                        first_message_id: firstMessageId
                    },
                    success: function(shouldYouLoadButton) {
                        console.log(shouldYouLoadButton);
                        if (shouldYouLoadButton.trim() == "false") {
                            let loadMoreButton = document.getElementById("load-more-messages");
